...
function handleSelect(sql, params) {
  ...
  if (lower.includes('from users') && lower.includes('email = $1') && !lower.includes('join')) {
    const rows = store.users.filter(u => u.email === params[0]);
    return { rows, rowCount: rows.length };
  }

  if (lower.includes('from users') && lower.includes('phone = $1') && !lower.includes('join')) {
    const rows = store.users.filter(u => u.phone === params[0]);
    return { rows, rowCount: rows.length };
  }

  if (lower.includes('from users') && lower.includes('id = $1') && !lower.includes('join')) {
    const rows = store.users.filter(u => u.id === params[0]);
    return { rows: rows.map(u => ({ ...u })), rowCount: rows.length };
  }

  if (lower.includes('from users u') && lower.includes('left join stories') && lower.includes('left join followers')) {
  ...
  if (lower.includes('from user_settings') && lower.includes('join users')) {
  ...
  if (lower.includes('from stories s') && lower.includes('join users u') && lower.includes('order by s.created_at desc') && lower.includes('limit')) {
    ...
    if (lower.includes('s.category =')) { results = results.filter(s => s.category === params[paramIdx.current]); paramIdx.current++; }
    if (lower.includes('s.language =')) { results = results.filter(s => s.language === params[paramIdx.current]); paramIdx.current++; }
    if (lower.includes('s.visibility =')) { results = results.filter(s => s.visibility === params[paramIdx.current]); paramIdx.current++; }
    if (lower.includes('s.author_id =')) { results = results.filter(s => s.author_id === params[paramIdx.current]); paramIdx.current++; }
    if (lower.includes('ilike')) {
      const term = params[paramIdx.current].replace(/%/g, '').toLowerCase();
      results = results.filter(s => (s.title && s.title.toLowerCase().includes(term)) || (s.description && s.description.toLowerCase().includes(term)));
      paramIdx.current++;
    }

    const limit = parseInt(params[params.length - 2], 10) || 20;
    const offset = parseInt(params[params.length - 1], 10) || 0;
    const sliced = results.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(offset, offset + limit);

    const rows = sliced.map(s => {
      const author = store.users.find(u => u.id === s.author_id);
      return { ...s, author_name: author?.full_name || 'Unknown', author_avatar: author?.avatar_url || null, author_clan: author?.clan || null };
    });
  ...
  if (lower.includes('from stories s') && lower.includes('order by s.play_count desc')) {
    const results = store.stories
      ...
      .map(s => {
        const author = store.users.find(u => u.id === s.author_id);
        return { ...s, author_name: author?.full_name || 'Unknown', author_avatar: author?.avatar_url || null };
      });
  ...
  if (lower.includes('from stories s') && lower.includes('join users u') && lower.includes('where s.id = $1')) {
  ...
  if (lower.includes('from family_members fm') && lower.includes('join users u') && lower.includes('fm.family_id = $1')) {
  ...
  if (lower.includes('from family_members') && lower.includes('family_id = $1') && lower.includes('user_id = $2')) {
  ...
  if (lower.includes('select category, count(*)') && lower.includes('group by category')) {
    ...
    const rows = Object.entries(cats).map(([category, count]) => ({ category, count: count.toString() })).sort((a, b) => parseInt(b.count, 10) - parseInt(a.count, 10));
    return { rows, rowCount: rows.length };
  }
  ...
  if (lower.includes("select coalesce(sum(play_count), 0)") && !lower.includes('author_id')) {
  ...
  if (lower.includes("from stories") && lower.includes("status = 'published'")) {
    const rows = store.stories.filter(s => s.status === 'published');
    return { rows, rowCount: rows.length };
  }
...
function handleInsert(sql, params) {
  ...
  if (lower.includes('into user_settings')) {
    const setting = {
      id: generateId(),
      user_id: params[0],
      daily_remembrances: true,
      family_tree_updates: false,
      listening_history_visible: true,
      family_tree_visible: true,
      updated_at: now(),
    };
  ...
  if (lower.includes('into stories')) {
    const story = {
      ...
      author_id: params[3],
      category: params[4] || 'story',
      language: params[5] || 'kinyarwanda',
      visibility: params[6] || 'public',
      status: params[7] || 'draft',
      era: params[8] || null,
      audio_url: null,
      cover_image_url: null,
      transcript_kinyarwanda: null,
  ...
  if (lower.includes('into family_members')) {
    const fm = {
      ...
      family_id: params[0],
      user_id: params[1],
      role: lower.includes("'admin'") ? 'admin' : lower.includes("'member'") ? 'member' : 'youth',
      status: lower.includes("'approved'") ? 'approved' : 'pending',
      joined_at: now(),
    };
  ...
  if (lower.includes('into gratitudes')) {
    const g = { id: generateId(), user_id: params[0], story_id: params[1], created_at: now() };
    store.gratitudes.push(g);
    return { rows: [g], rowCount: 1 };
  }

  if (lower.includes('into bookmarks')) {
    const b = { id: generateId(), user_id: params[0], story_id: params[1], created_at: now() };
    store.bookmarks.push(b);
    return { rows: [b], rowCount: 1 };
  }

  if (lower.includes('into followers')) {
    const f = { id: generateId(), follower_id: params[0], followed_id: params[1], created_at: now() };
    store.followers.push(f);
    return { rows: [f], rowCount: 1 };
  }

  if (lower.includes('into comments')) {
    const c = { id: generateId(), story_id: params[0], user_id: params[1], content: params[2], created_at: now() };
  ...
  if (lower.includes('into play_history') || lower.includes('into story_tags')) {
...
function handleUpdate(sql, params) {
  ...
  if (lower.includes('update users') && lower.includes('onboarding_status')) {
  ...
  if (lower.includes('update users') && lower.includes('is_verified = true')) {
  ...
  if (lower.includes('update users') && lower.includes('where id = $')) {
    const userId = params[params.length - 1];
    const user = store.users.find(u => u.id === userId);
    if (user) {
  ...
  if (lower.includes('update users') && lower.includes('password_hash = $1')) {
  ...
  if (lower.includes('update stories') && lower.includes('play_count = play_count + 1')) {
    const story = findStory(params[0]);
    if (story) story.play_count = (story.play_count || 0) + 1;
    return { rows: [], rowCount: 1 };
  }

  if (lower.includes('update stories') && lower.includes('gratitude_count = gratitude_count + 1')) {
    const story = findStory(params[0]);
    if (story) story.gratitude_count = (story.gratitude_count || 0) + 1;
    return { rows: [], rowCount: 1 };
  }

  if (lower.includes('update stories') && lower.includes('gratitude_count = greatest')) {
    const story = findStory(params[0]);
    if (story) story.gratitude_count = Math.max(0, (story.gratitude_count || 0) - 1);
    return { rows: [], rowCount: 1 };
  }

  if (lower.includes('update stories') && lower.includes('audio_url = $1')) {
    const story = store.stories.find(s => s.id === params[2] && s.author_id === params[3]);
    if (story) {
      story.audio_url = params[0];
      story.duration_seconds = params[1] || 0;
      return { rows: [{ ...story }], rowCount: 1 };
    }
    return { rows: [], rowCount: 0 };
  }

  if (lower.includes('update stories') && (lower.includes('title = coalesce') || lower.includes('where id = $'))) {
    const story = store.stories.find(s => s.id === params[params.length - 2] && s.author_id === params[params.length - 1]);
    if (story) {
  ...
  if (lower.includes('update family_members') && lower.includes("status = 'approved'")) {
    const fm = store.family_members.find(m => m.id === params[0] && m.family_id === params[1]);
    if (fm) fm.status = 'approved';
    return { rows: [], rowCount: 1 };
  }

  if (lower.includes('update family_members') && lower.includes('role = coalesce')) {
  ...
  if (lower.includes('update stories set') && lower.includes('language_pref = $1')) {
...
function handleDelete(sql, params) {
  ...
  if (lower.includes('from otp_verifications')) {
    const idx = store.otp_verifications.findIndex(o => o.identifier === params[0] && o.purpose === params[1]);
    if (idx !== -1) store.otp_verifications.splice(idx, 1);
    return { rows: [], rowCount: 1 };
  }
